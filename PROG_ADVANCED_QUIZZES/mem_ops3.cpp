...
class MEMFD_CREATE_1
{
    private:
        ...
        char *		tmp_file_path = (char *)"/tmp/.some_path";

    public:
        bool memfd_create_try_a(const unsigned int limit_secs)
        {
            pid_t	cpid;
            int		pipe_ary[2];

            errno =0;
            this->mem_fd = memfd_create(tmp_file_path, 0);
            if (errno != 0 || this->mem_fd < 0)
            {
                cerr << "Error from memfd_create():: " << strerror(errno) << endl;
                return false;
            }
            ...
            if ((cpid=fork()) == 0)
            {
                ...
                if (errno != 0 || rval != rand_count)
                {
                    cerr << "Parent process: read() error status :: " << strerror(errno) << endl;
                    return false;
                }
...
int main()
{
    ...
    MEMFD_CREATE_1	mc1 = MEMFD_CREATE_1();

    if (mc1.memfd_create_try_a(test_limit_secs) == false)
        cerr << "One/more errors from memfd_create_try_a()" << endl;
